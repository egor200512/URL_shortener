		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.setupLink {
				_, err := pool.Exec(ctx, `
                    ...
                    VALUES ($1, $2, $3, $4, $5, $6)`,
					uuid.New(), uuid.New(), tt.shortLink, "example.com", "example.com/path", time.Now().UTC())
				require.NoError(t, err)
			}

			link, err := repo.GetByShortLink(ctx, tt.shortLink)

			assert.NoError(t, err)
			if tt.wantLink {
				assert.NotNil(t, link)
				assert.Equal(t, tt.shortLink, link.ShortLink)
				assert.Equal(t, "example.com", link.OriginalLinkHost)
				assert.Equal(t, "example.com/path", link.OriginalLink)
